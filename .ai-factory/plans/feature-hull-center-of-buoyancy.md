...
> Устаревший план: этот артефакт описывает `ЦВ` как центр величины и не является источником требований для `ЦВК` (цилиндрической вставки корпуса). Реализация `ЦВК` ведется отдельно в `fix-cvk-terminology-and-geometry.md`; расчет центра величины должен возвращаться позже как отдельная задача баланса.
