...
- `ProfileSnapshot` хранит compatibility/display `radius` и точные полуоси сечения `halfBreadthY`/`halfHeightZ`; legacy 3D mesh строится как exact elliptical ring mesh.
...
- Расчет радиуса корпуса по нормированной формуле `r(s) = D/2 * sqrt(f(s/L) / fmax)`, где `f(t) = t * (1 - t) * (1 - 0.5*t)`, а `D` — физический максимальный диаметр корпуса.
- Выбор режима геометрии корпуса: `current-formula` по умолчанию или `legacy-dsnp-pa` с DSNP_PA `MaxWl`/`MaxBt` traceability и эллиптическими сечениями.
