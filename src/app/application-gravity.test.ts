...
import { addEquipmentItem } from "../modules/equipment/placement";
...
describe("application gravity workflow", () => {
  it("preserves imported gravity through unrelated store updates and resets it with project reset", () => {
    ...
    store.replaceProject(projected.inputs);

    store.setEquipment(addEquipmentItem(store.getSnapshot().equipment));
    ...
    store.replaceProject(createDefaultProjectInputs());
