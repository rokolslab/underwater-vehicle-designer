...
| `Координаты точек` | Таблица станций и CSV export |
...
| `Удлинение lambda = L / D` | Связь длины и диаметра | `>= 0.1` |
| `Диаметр D` | Максимальная физическая высота корпуса | `>= 0.01` |
...
Связь `D = L / lambda` двунаправленная:

- если пользователь редактирует `lambda`, пересчитывается `D`;
- если пользователь редактирует `D`, пересчитывается `lambda`.

## Боковой вид

Панель показывает профиль корпуса в координатах `x/y`.
...
| --- | --- | --- |
| `Режим` | `Сплошной`, `Рентген`, `Вырез` | Меняет материал и видимость внутренней компоновки |
...
| `Масса` | Масса в кг |
| `X`, `Y`, `Z` | Центр объекта в координатах корпуса |
| `Ось` | Главная ось цилиндра: `X`, `Y`, `Z` |
| `Р`, `Дл.`, `Ш`, `В`, `Гл.` | Размеры, зависят от формы |
...
| `ЦВ - ЦТ` | Плечо между центрами |

Предупреждения выводятся внизу панели.
...
| `y верх` | Верхняя ордината |
| `y низ` | Нижняя ордината |
