...
`B` редактируется независимо. В строке режима показывается короткое имя режима (`Текущая` или `ДСНП_ПА`) и активная формула/traceability: текущая формула масштабирует один shape factor по `B/H`, legacy mode использует `MaxWl(B)`/`MaxBt(H)`.
...
Сечения задаются в Body: `X ∈ [-L/2,+L/2]`, плоскость `XY` означает `z = offset`, `XZ` — `y = offset`. Clipping сохраняет половины `x <= offset`, `z <= offset` или `y <= offset` соответственно; пределы offset используют `H/2` для `XY` и `B/2` для `XZ`. Оси в сцене подписаны: X — нос, Y — правый борт, Z — вниз.
