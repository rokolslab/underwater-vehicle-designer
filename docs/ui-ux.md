...
Сечения задаются в Body: `X ∈ [-L/2,+L/2]`, плоскость `XY` означает `z = offset`, `XZ` — `y = offset`. Clipping сохраняет половины `x <= offset`, `z <= offset` или `y <= offset` соответственно. Оси в сцене подписаны: X — нос, Y — правый борт, Z — вниз.
