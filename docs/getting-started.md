...
## Public Demo v1 Scope

Для публикации сайта проект фиксируется как `Public Demo v1`: интерактивная визуализация корпуса является основой продукта, а остальные блоки показывают текущие проверяемые возможности прототипа. Пользовательский сценарий должен быстро приводить к 2D/3D-просмотру, изменению размеров корпуса и сохранению результата.

В scope демо входят текущие рабочие функции: режимы геометрии, ввод `L/B/H/lambda/ЦВК`, боковой вид, 3D-сцена, таблица точек, теоретический чертеж, базовая компоновка оборудования, equipment-only баланс и экспорт/импорт проекта.

Не входит в scope демо: новые расчетные модели корпуса, полный ЦВ герметичного корпуса, инерция, массовые группы, ходкость, энергетика, стоимость, расширенный legacy-импорт и deploy на конкретный домен. Эти направления остаются backlog и не должны описываться как готовые возможности сайта.
...
2. В панели `Размерения` задайте длину, удлинение, ширину, высоту и ЦВК.
