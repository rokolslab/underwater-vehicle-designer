...
Принята правая декартова система:

| Axis | Meaning |
| --- | --- |
| `x` | Продольная координата от носа к корме |
| `y` | Вертикальная координата в боковом виде |
| `z` | Поперечная координата в плане/3D |

Корпус считается телом вращения вокруг оси `x`. Радиус сечения в точке `x` ограничивает допустимые значения `sqrt(y^2 + z^2)`.
...
| Reason | Condition |
| --- | --- |
| `outsideLength` | `minX < 0` или `maxX > totalLength` |
...
Для цилиндров и блоков используется несколько контрольных точек по длине extents.
...
| `unstableVerticalCenters` | `CB.z <= CG.z` |
