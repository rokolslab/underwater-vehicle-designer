...
| Mode | Behavior |
| --- | --- |
| `current-formula` | `Базовая формула`, режим по умолчанию. Один нормированный shape factor масштабирует `B/2` и `H/2`, поэтому при `B != H` сечения эллиптические. |
| `legacy-dsnp-pa` | `Классическая методика`, traceability-режим по материалам `APPAUNIT.PAS`: `B -> MaxWl`, `H -> MaxBt`. Это не доказательство инженерной валидности исторических коэффициентов. |
